...
from filldoc.ui.tabs.templates_tab import _apply_output_name_rule


class TestApplyOutputNameRule:
    def test_filename_token(self) -> None:
        result = _apply_output_name_rule("{%filename%}", "Заявление", {})
        assert result == "Заявление"

    def test_debtor_token(self) -> None:
        result = _apply_output_name_rule(
            "{%filename%} - {ДОЛЖНИК}",
            "Заявление",
            {"ДОЛЖНИК": "ООО Ромашка"},
        )
    ...
    def test_fallback_when_rule_is_empty(self) -> None:
        result = _apply_output_name_rule("", "Шаблон", {})
